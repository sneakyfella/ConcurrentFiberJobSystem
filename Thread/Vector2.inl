...
#include "Assert.h"
    ...
    {
        ASSERT(index < 2, "Accessing beyond index bounds."); // Halt the program if this exceeds.
    ...
    {
        ASSERT(false,"Normalise() not defined for this type.");
    }

    template< typename T >
    inline const typename Vector2<T> Vector2<T>::GetNormalised(void) const
    {
        ASSERT(false, "GetNormalise() not defined for this type.");
    ...
    {
        return Math::Sqrt<Real>(GetSquaredLength());
    }
    ...
    {
        return Math::Sqrt<Real>(GetSquaredDistance(rhs));
    }
